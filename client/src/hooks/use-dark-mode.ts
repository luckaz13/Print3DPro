...
export function useDarkMode(): DarkModeState {
  ...
  const [isDark, setIsDarkState] = useState(() => {
    if (typeof window === 'undefined') return true; // Dark mode por padrão no SSR
    
    const savedMode = localStorage.getItem('darkMode');
    
    // Se não há preferência salva, usar Dark Mode por padrão
    if (savedMode === null) {
      return true; // Dark mode por padrão
    }
